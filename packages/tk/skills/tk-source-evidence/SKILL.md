...
description: Use when a TK answer needs implementation evidence from cloned source projects, local source-cache freshness checks, source sync plans, file/function citations, or when the user asks the agent to clone, pull, download, prepare, sync, or check the TK reference repositories/source cache. Trigger for phrases like "把 TK 需要的仓库 clone 下来", "拉报告里的源码", "prepare TK source repos", and "sync missing TK sources".
---
...
3. Read targeted source files with the host's native search and file-reading
   tools.
