...
> 一句话定位：`bytedance/UI-TARS-desktop` 已从单一 UI-TARS 桌面应用演化为 ByteDance 的 **Multimodal AI Agent Stack** 仓库：同仓承载 Agent TARS、UI-TARS Desktop、GUI Agent SDK、Operator 家族、browser / MCP / remote infra。若聚焦 UI-TARS Desktop，它最有价值的部分不是 Electron 壳，而是 `GUIAgent Loop + VLM Model Adapter + Action Parser + Operator Port` 这套可复刻的 GUI / computer-use runtime。
...
| Star | 36,594（2026-06-17 GitHub API 快照） |
| Fork | 3,697（2026-06-17 GitHub API 快照） |
| Watchers | 269（2026-06-17 GitHub API 快照） |
| 许可证 | Apache-2.0 |
...
| 当前 HEAD | `e9f3387288da` / 2026-05-18，`chore: license (#1895)` |
| GitHub pushed_at | 2026-05-18T02:55:49Z |
| 最新 GitHub Release | `v0.3.0` / 2025-11-04；注意该 tag 在 README 中对应 Agent TARS CLI，不应简单等同 UI-TARS Desktop App 版本 |
| UI-TARS Desktop App 版本 | `apps/ui-tars/package.json` = `0.2.4` |
| `@ui-tars/*` 包版本 | `packages/ui-tars/*/package.json` 多为 `1.2.3` |
| 贡献者 | GitHub contributors API 当前页 48；本地 `git shortlog` 52 |
| Issue / PR | repo API `open_issues_count=404` 含 PR；搜索快照：open issue 319、closed issue 216、open PR 85、merged PR 1073 |
| 仓库体量 | 2,491 tracked files；TypeScript 1,158，TSX 282，MD/MDX 310，JSON 236；test-like tracked files 约 163（排除 benchmark result 噪音） |
...
旧报告结论“观望（生产）/ 推荐（PoC、研究、架构学习）”仍成立，但项目边界和风险口径需要更新：

1. **仓库定位变宽**：README 当前第一句已是 `TARS is a Multimodal AI Agent stack`，并把 Agent TARS 与 UI-TARS Desktop 并列。报告不能再把仓库只写成 UI-TARS Desktop App。
2. **UI-TARS Desktop 仍是 GUI automation 核心样本**：`apps/ui-tars` + `packages/ui-tars/sdk` + `operators/*` 的主链没有变，`GUIAgent.run()` 仍是最值得学习的心脏。
3. **指标继续增长**：Star 从旧报告的 31.4k 增到 36.6k，Fork 从 3.1k 增到 3.7k，热度持续。
4. **源码主分支最近 push 在 2026-05-18**：分析日的 GitHub open PR 仍很多，但默认分支代码本身不是“每天都在大改”。
5. **Release 口径必须拆开**：GitHub latest release `v0.3.0` 更偏 Agent TARS CLI；UI-TARS Desktop App package 是 `0.2.4`。选型时要分清 “TARS stack release” 与 “Desktop app package”。
6. **安全与可审计需求上升**：open PR 已出现 ProofPilot、command safety approval gate、agent security hardening、Host-header validation 等主题，说明社区正把 GUI agent 从 demo 推向“可审计 / 可控执行”。
7. **文档漂移仍存在**：`docs/quick-start.md` 写 Remote Operator service 将于 2025-08-20 discontinued，但 README 又保留 Remote Computer / Remote Browser 的产品叙事，源码也仍有 remote proxy/operator。真实边界必须以源码 + 当前服务状态复核。
...
- 不是企业生产级 RPA 替代品：缺少任务可重放、强审计、安全审批、策略沙箱、rollback、deterministic execution plan 等生产 RPA 必需件。open issue / PR 中的 workflow replay、approval gate、ProofPilot 正说明这些还在补。
...
| Bus factor | 中 | local shortlog 52 contributors，头部贡献集中在 ULIVZ、Charles、heh、chenhaoli 等；不是单人项目，但方向由核心团队主导 |
| 供应商 / backend 锁定 | 中 | SDK 支持 OpenAI-compatible API，但 UI-TARS action 质量依赖特定 VLM；remote operator 依赖 `/api/v1` backend contract |
| 维护趋势 | 活跃但主分支节奏放缓 | Star/PR/Issue 活跃；默认分支最近 push 2026-05-18，open PR 多，说明社区活跃但 main 合入需要观察 |
| 安全攻击面 | 中-高 | 本地屏幕、键鼠控制、浏览器、远程沙箱、设备注册、JWT auth、model API key、IPC、日志都在攻击面内 |
| 日志泄露风险 | 中 | `runAgent.ts` 的 `onError` 会把 `settings` 打入日志，而 settings schema 包含 `vlmApiKey`；需要生产化前做脱敏 |
| GUI 执行稳定性 | 中-高 | issue 中已有 click marker offset、app 置顶影响操作、mozjpeg prebuild、黑屏等问题；GUI 自动化天然脆弱 |
| 文档漂移 | 中 | quick-start / release / app package / remote operator 口径不完全一致 |
| 测试可信度 | 中-高 | CI 覆盖 typecheck、coverage、E2E、secret scan、scorecard；但本次未运行，且 GUI/E2E 对平台环境依赖强 |
...
- **活跃度：** open issue 319、open PR 85、merged PR 1073，说明社区仍在高频参与。
- **近期 open issue 样本：** workflow export/replay、Dia Browser/profile、官网打开黑屏、click prediction marker offset、app 置顶导致操作无效、mozjpeg prebuild failure。
- **近期 open PR 样本：** docs stale FIXME、MCP subpath builds、VLM provider tests、MiniMax provider、ProofPilot auditable/simulation-first agent execution、command safety approval gate、agent security checks。
- **解读：** 用户真实痛点集中在“稳定执行、可审计、安全、可重放、provider 扩展、浏览器 profile”。这和报告采用结论一致：PoC 强，生产化还要补控制面。
...
- **热度高且持续增长**：36.6k stars / 3.7k forks，在 GUI Agent / computer-use / multimodal agent 方向属于头部开源项目。
- **社区不是只看概念**：open issues 更关心 offset、黑屏、置顶、prebuild、browser profile、workflow replay 这些真实落地问题。
- **维护方向在从“能动”转向“可控”**：ProofPilot、command safety approval gate、agent security checks 这类 PR 说明下一阶段重点会是审计、安全、模拟、审批。
- **生态更多是内生而非第三方插件市场**：`@ui-tars/*`、`@agent-infra/*`、Agent TARS / Tarko / Omni-TARS 都在同仓内生长；外围扩展信号没有 browser-use 那么分散。
...
|------|-------|-------|-----------|------|
| `browser-use/browser-use` | 99,262 | 11,081 | 2026-06-15 | 浏览器自动化头部项目；更聚焦 browser |
| `All-Hands-AI/OpenHands` | 77,506 | 9,853 | 2026-06-17 | Coding agent 平台；不是 GUI runtime 直接竞品 |
| `OpenInterpreter/open-interpreter` | 64,030 | 5,555 | 2026-06-17 | 本地系统控制/解释器生态；邻近替代 |
| `bytedance/UI-TARS-desktop` | 36,594 | 3,697 | 2026-05-18 | GUI / computer-use runtime 代表项目 |
| `mediar-ai/screenpipe` | 19,335 | 1,840 | 2026-06-17 | 屏幕上下文 / personal data infra |
| `microsoft/TaskWeaver` | 6,165 | 779 | 2026-03-23 | 已 archived；更多是历史参照 |
...
| 社区活跃度 | 5 | 36.6k stars、3.7k forks、open PR/issue 很活跃，仍是 GUI Agent 方向头部项目 |
...
UI-TARS-desktop 是当前开源 GUI Agent / computer-use runtime 的代表样本：**非常值得学，适合 PoC 和抽取 SDK/Operator 思路；但直接作为生产桌面自动化底座仍需观望和二次 hardening。**
