...
| 核心强项 | LLM Provider + RAG + MCP/tools + admin/distribution | Memory Tree + tool registry + workflow runtime + run ledger + connectors | GUIAgent loop、VLM adapter、action parser、Operator port、本地/浏览器/远程执行 |
| 功能覆盖度 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| 集成成本 | ⭐⭐⭐⭐ | ⭐⭐ | ⭐⭐ |
| 社区健康 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| 文档质量 | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ | ⭐⭐⭐⭐ |
...
- **适合采用的点**：本地优先 desktop personal AI OS，Memory Tree、Obsidian vault、agent tool registry、MCP、Composio、workflow/skill runtime、session_db/run ledger、多 Agent orchestration、voice/Meet/channel 等方向完整，架构密度极高。
- **主要风险**：GPL-3.0；默认 managed backend 仍承担账号、模型路由、web search proxy、Composio OAuth / managed integration flows；桌面/Tauri/CEF/Rust core + 宽工具面 + MCP/Composio/voice/screen/web3/x402 攻击面重；项目仍处 early beta 和高速修补。
...
- **主要风险**：不是通用个人 AI 助手管理台；GUI 自动化在安全、可复现性、跨平台稳定性、日志脱敏和执行审计上天然复杂；README / release / app package / remote operator 口径需要单独核验。
...
| 能力组合根 | Store 绑定 prompt/provider/RAG/tools/MCP | Controller/tool/provider/MCP/workflow/profile registries | GUIAgent loop + Model adapter + Action parser + Operator port |
...
| 工具体系 | builtin tools + MCP tools 合并 ToolSet | tool registry + MCP client/server + workflow runtime + security policy | 本地电脑、浏览器、远程电脑/浏览器等 Operator family |
...
5. **openhuman 的 registry-first platform kernel**
   - Controller、Tool、Provider、MCP、Workflow、Profile 都 registry 化，能力可发现、可过滤、可授权。
...
7. **openhuman 的 workflow/skill runtime 终态契约**
   - `DONE / FAILED / CANCELLED / DEGENERATE` footer、preflight gate、cancel token、repeated-line detector，是 agent 长任务产品化的关键模式。
...
- **学 agent workflow runtime / durable long-task ledger** → 重点读 **openhuman** 的 `skill_runtime`、`session_db`、`agent_orchestration`。
...
- **openhuman** 教你怎么做本地优先 personal AI OS：desktop lifecycle、RPC control plane、tool policy、Memory Tree、workflow runtime、run ledger。
- **UI-TARS-desktop** 教你怎么做 GUI agent / desktop automation runtime。

### 综合冠军

- **采用冠军：openagent（PoC/个人自托管场景）**
- **架构学习冠军：openhuman（平台深度、长期记忆、workflow runtime） + openagent（产品闭环）并列**
