...
Technical Knockout（TK）是一套面向开源项目技术选型与架构学习的分析方法。它的目标不是“写一篇项目介绍”，而是产出可复用、可比较、可被 AI Agent 消化的技术研究资产。

## 分析目标

每份 TK 报告都要同时服务两个场景：

1. **采用选型**：这个项目是否值得引入？适合个人、小团队、企业生产化，还是应该观望？
2. **架构学习**：这个项目为什么能成立？如果重写同类系统，哪些抽象、执行链路和设计不变量必须保留？
...
- 目录结构和核心模块职责。
- 技术栈、运行时、构建方式、测试工具、CI/CD。
- 模块依赖关系。
- 扩展机制：插件、Hook、Provider、配置、协议、Skill、MCP 等。
