...
> **一句话定位**：CodeGraph 是一个 MIT 许可、本地优先的代码知识图谱库 + CLI + MCP Server，用 tree-sitter/WASM 把项目索引成 SQLite/FTS5 符号图，再让 Claude Code、Cursor、Codex、OpenCode、Hermes Agent、Gemini、Antigravity、Kiro 等 Agent 用少量工具调用完成代码探索。
...
| Star | 58,226（观测于 2026-07-07） |
| Fork | 3,593 |
| 许可证 | MIT |
| 语言 | TypeScript |
| 首次提交 | 2026-01-18 |
| 最近提交 | 2026-07-06（`6ea6524 readme updated`） |
| 最新 Release | `v1.2.0`（2026-07-02） |
| npm | `@colbymchenry/codegraph@1.2.0`，最近 30 天下载约 384,390 |
| 贡献者信号 | API Top 10 中核心：`colbymchenry` 573；其后贡献明显长尾化 |
| 分析日期 | 2026-07-07 |
...
README 现在把收益表述得更克制也更可信：在 7 个真实开源代码库的基准中，**58% fewer tool calls、22% faster、file reads 近乎归零** 是更稳定的收益；token / cost 节省仍存在，但被明确标成**规模相关**收益，而不是任何仓库都能线性兑现。

### 核心能力与边界

**能做什么：**

- 用 `install.sh` / `install.ps1` 走**bundled runtime** 安装（README 明写 *No Node.js required*），或用 npm 全局安装；再通过 `codegraph install` 自动配置 Claude Code、Cursor、Codex、OpenCode、Hermes Agent、Gemini、Antigravity、Kiro 等宿主。
- `codegraph init` 在项目本地生成 `.codegraph/codegraph.db` 并完成首轮建图。
- 用 `web-tree-sitter` / WASM grammar 解析 30+ 语言/方言与常见 Web 框架，不再只是早期那批主流语言。
...
- 默认开启 auto-sync：文件保存后 watcher 增量更新，WSL `/mnt` 等环境再退回 git hook / manual sync。
- 支持 bundled runtime / npm 安装双路径；SQLite 后端仍是 native 优先、fallback 到 wasm。
...
- 仓库内 workflow 仍偏轻：现在已有 `release.yml` / `deploy-site.yml`，但还不是一套完整的 repo-level CI / security gate。
...
- **依赖链**：两条路径。用户侧推荐 bundled installer（**不要求本机先装 Node**）；npm 路径则要求 Node.js `>=20 <25`。核心运行时依赖仍是 `web-tree-sitter`、`tree-sitter-wasms`、`picomatch`、`jsonc-parser`、`@clack/prompts` 等。
- **部署复杂度**：低。最短路径已经变成 `curl .../install.sh | sh` → `codegraph install` → `codegraph init`；不再强迫用户先处理 Node/npm 环境。
- **学习曲线**：低到中。用户只要理解“install 接宿主、init 建索引、之后交给 MCP 工具”；但要用好 `context` vs `explore` vs `node` 的边界，仍得看 server instructions。
- **从零到 demo**：小项目 3-5 分钟；大项目主要成本在首次索引和 native/wasm SQLite 路径选择。
- **运行约束**：npm 路径仍受 Node 20-24 约束；fallback 到 wasm SQLite 虽能保活，但性能和锁行为会明显变差。
...
| Bus factor | ⚠️ 中高 | Star / npm 动能非常强，但提交仍高度集中在 `colbymchenry`；open PR 前 100 条就已打满，维护压力真实存在。 |
| 供应商锁定 | ✅ 低 | 数据本地 SQLite，MCP 是开放协议；Agent 配置可卸载，bundled installer 也不改变“本地优先”边界。 |
| 维护趋势 | ✅ 很活跃 | 稳定版已到 `v1.2.0`，README、站点、宿主支持和发布都在持续推进，说明项目已越过早期试验期。 |
| 安全历史 | ⚠️ 中 | 当前 `picomatch@4.0.3` 仍命中两条 advisory：一个 moderate method injection、一个 high ReDoS；fix 版本已存在（`4.0.4`）。 |
| 质量 gate | ⚠️ 中 | 仓库现在有 `release.yml` / `deploy-site.yml`，但仍缺少更完整的 repo-level test / security CI 面。 |
| 产品演进速度 | ⚠️ 中 | 1.0 发布后很快到 1.2.0，同时开始强调 hosted platform / waitlist；对用户是利好，对企业标准化则意味着要防接口和分发面继续快速变化。 |

### 结论

**推荐采用（个人/小团队/内部受控 PoC） / 企业核心强制标准件暂时观望。**

理由：

1. **定位依旧精准**：它不是“又一个 coding agent”，而是给现有 coding agent 装一个本地结构化索引层。
2. **MIT + 本地优先 + 安装更顺滑**：相比 GitNexus 的非商业许可，CodeGraph 更适合作为团队可控基础件试用；bundled installer 又把门槛进一步压低。
3. **产品成熟度比 5 月高很多**：`v1.2.0`、384k 月下载、58k stars、更多宿主支持，说明它已经越过“新鲜 demo”阶段。
4. **工程方向是对的**：strict TS、worker parse、WAL/lock、watch fallback、installer idempotency、server instructions 这些都在解决真实 Agent 探索痛点。
5. **但离企业默认标准件仍差一层**：依赖安全红点、CI 面偏轻、bus factor 高、open PR backlog 大，且产品开始平台化扩张。

我会把它放进“默认可试工具箱”的更靠前位置：给个人 Claude/Codex/Cursor/Hermes 项目直接装上，观察真实大仓库里的命中率、误报率和索引稳定性；如果要在企业强制推广，先补安全依赖、CI 策略、内部 benchmark 和安装链路审计。
...
CodeGraph 的最小内核是 **本地文件扫描 + Tree-sitter/WASM 解析 worker + SQLite nodes/edges/FTS 图谱 + best-effort resolution + MCP 工具面与 server instructions + installer target contract + watch/sync**。它不需要云服务或大型图数据库，但必须保证本地索引、结构化查询和 Agent 使用策略一起交付。

#### 核心抽象

- `CodeGraph` facade：统一 library、CLI、MCP 的 lifecycle、index、sync、query、watch。
- `Node` / `Edge` / `File` / `UnresolvedRef`：SQLite 图谱的核心持久对象。
- `Language` / grammar：通过 `web-tree-sitter` 和 WASM grammar 解析多语言 AST。
- `Resolver`：按 framework rule、import alias、name matching 建立调用和引用边。
- `MCP Tool`：`search/context/callers/callees/impact/node/explore/status/files` 的 Agent 查询面。
- `AgentTarget`：多宿主 detect/install/uninstall/printConfig/describePaths 的安装契约。
- `ExploreBudget` / `FileWatcher`：控制输出成本和增量同步策略。

#### 控制面 / 数据面

- **控制面**：CLI init/install/sync/status、`.codegraph/config.json`、installer target registry、MCP server instructions、explore budget、watch fallback 策略。
- **数据面**：文件扫描、Tree-sitter worker 解析、SQLite nodes/edges/FTS 写入、query traversal、MCP 工具结果、宿主配置文件写入。

#### 关键执行链路

1. **建图**：`codegraph init` 创建 `.codegraph/`，扫描 include/exclude 文件，worker 解析 AST，提取符号和关系，resolver 生成 edges，写入 SQLite + FTS5。
2. **Agent 查询**：MCP initialize 返回 server instructions，Agent 调 `codegraph_context` 或 `codegraph_explore`，工具从 SQLite 图谱和源码片段构造 line-numbered 输出，避免先 grep/read。
3. **宿主安装**：`codegraph install` 检测 Claude/Cursor/Codex/OpenCode 等 target，通过各自 adapter 写 JSON/TOML/JSONC 配置和说明，支持卸载与路径说明。

#### 状态模型

- **持久状态**：`.codegraph/codegraph.db`、FTS tables、`.codegraph/config.json`、宿主 MCP 配置。
- **运行时状态**：parse worker、WASM grammar memory、watcher debounce、open DB connection、MCP request context。
- **外部状态**：项目文件系统、git hooks/manual sync、Agent host config、Node native/wasm SQLite 可用性。SQLite 图谱是查询事实源，但必须通过 watch/sync 与文件系统重新对齐。

#### 契约边界

- **内部契约**：`CodeGraph` public API、SQLite schema、resolver input/output、parse worker timeout/recycle。
- **外部契约**：`codegraph` CLI、MCP 9 类工具、server-level instructions、installer `AgentTarget` interface。
- **Agent-facing 契约**：工具输出必须带文件路径和行号，server instructions 直接约束 Agent 先用 CodeGraph 而不是盲目文件探索。

#### 失败与降级模型

- Tree-sitter/WASM 解析有 timeout、worker recycle、OOM retry、comment-stripping retry，避免单文件拖死 MCP。
- WSL `/mnt` 等环境 watcher 不可靠时退回 git hook 或 manual sync。
- native SQLite 不可用时可 fallback 到 wasm SQLite，但 status 需要暴露性能降级。
- 调用解析是 best-effort，动态语言、多态、运行时注入仍需 Read/Grep/测试验证。
- 文件片段读取必须经过 `validatePathWithinRoot()` 防 path traversal。

#### 可复刻设计不变量

1. 本地代码智能优先用轻量持久图谱证明价值，未必需要大图数据库。
2. Agent 工具必须连同使用策略一起通过 MCP instructions 交付。
3. WASM/parser 工作必须隔离在 worker，并有 timeout/recycle。
4. 输出预算应随项目规模变化，而不是统一截断。
5. 多宿主安装要有 target contract 和 contract tests。
6. 索引事实源与文件系统必须有 sync/watch/fallback 策略。
7. path traversal guard 是 MCP 读源码工具的硬边界。
8. best-effort resolution 要在产品文档里承认，而不是伪装成编译器。
...
1. **双分发路径会增加排障复杂度**
   - 现在既有 bundled installer / 自带 runtime，也有 npm 路径。
   - 对最终用户是好事，但一旦出现 PATH、upgrade、agent config 或平台差异问题，定位会比单一路径更复杂。
   - 企业内标准化时，最好先规定“只走 bundled”还是“只走 npm”。
...
3. **仓库内 workflow 仍偏轻**
   - 现在已有 `release.yml` 和 `deploy-site.yml`，说明发布与站点有自动化。
   - 但还缺少更完整的 test/security CI 面；随着 PR backlog 继续变大，这会成为维护压力放大器。
...
| 维度 | 评分(1-5) | 说明 |
|------|----------|------|
| 功能覆盖度 | 4.5 | 覆盖本地图谱、MCP、impact、context、watcher、installer、更多宿主支持；但没有 GitNexus 那种深图谱/跨仓库/Web UI/semantic hybrid。 |
| 代码质量 | 4.0 | strict TS、真实 SQLite 测试、worker/lock 设计扎实；扣分在大文件模块、配置枚举漂移、audit 风险。 |
| 文档质量 | 4.5 | README/网站/CHANGELOG/CLAUDE.md 已明显补齐，安装与收益叙事比 5 月更一致。 |
| 社区活跃度 | 4.5 | 58k⭐ / 384k 月下载 / 强宿主扩张动能都很亮眼；但 bus factor 高，PR backlog 也非常真实。 |
...
**CodeGraph 现在已经不只是“值得拿来试”的轻量本地 Code Intelligence MCP，而是这一赛道里最像默认试用标准件的项目之一：没有 GitNexus 那么深，但 MIT、简单、Agent-first，而且 7 月的产品完成度明显上了一个台阶。**
...
1. **作为个人/小团队工具，建议直接试用**：走 `install.sh`/`install.ps1` 或 npm 安装后跑 `codegraph install` + `codegraph init`，在 Claude/Codex/Cursor/Hermes 中对比原生 grep/read 和 CodeGraph MCP 的 tool-call 差异。
2. **如果要内部标准化**：先补内部安全基线：升级 `picomatch`、审计 bundled vs npm 分发策略、补 CI、写一套内部 benchmark。
3. **如果要学习架构**：重点读 `src/mcp/tools.ts`、`src/extraction/index.ts`、`src/resolution/index.ts`、`src/installer/targets/*`。
4. **如果要贡献 PR**：优先做低风险但价值高的修复：`picomatch` 升级、config language validation 对齐 `LANGUAGES`、repo-level CI baseline、bundled/runtime 路径文档与升级链测试。
