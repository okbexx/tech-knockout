...
Pi 的最小内核是 **`pi-ai` Provider Substrate + `pi-agent-core` Runtime + `pi-coding-agent` Session/Tool/Extension Shell + TUI + Supply-chain Release Discipline**。CLI 体验可以变化，但 provider abstraction、agent runtime、session tree、extension lifecycle 和安全发布纪律是它作为可嵌入 agent substrate 的核心。

#### 核心抽象

- `Provider`：OpenAI、Anthropic、Google、Bedrock 等统一模型调用面。
- `Agent`：transport、state、queues、tool execution 和 hooks 的 runtime core。
- `Session` / session entry：append-only jsonl、versioning、resume/fork/tree navigation 的持久单元。
- `Extension`：可注册 tools、commands、providers、lifecycle event 和 UI interaction。
- `Tool`：coding agent 的 read/bash/edit/write 等 side effect 能力。
- `Compat shim`：让旧 API 和全局 provider registry 平滑迁移到 side-effect-free core。
- `Release artifact`：shrinkwrap、exact deps、audit signatures、release smoke 共同构成分发契约。

#### 控制面 / 数据面

- **控制面**：provider registry/compat、Agent runtime queues、SessionManager、ExtensionRunner、compaction/branch policy、release scripts、lockfile/shrinkwrap/audit。
- **数据面**：模型 provider 请求、tool I/O、文件编辑、bash 执行、jsonl session append、TUI differential rendering。

#### 关键执行链路

1. **Coding session**：用户通过 CLI/TUI 启动 `pi-coding-agent`，产品层创建 session，调用 `pi-agent-core` 运行 tool loop，经 `pi-ai` provider substrate 请求模型，再把 tool result 和消息 append 到 session log。
2. **Provider 调用**：新代码走 side-effect-free `packages/ai/src/index.ts`，旧调用方由 `compat.ts` 兜底，统一 provider API 再分发到 OpenAI/Anthropic/Google/Bedrock 等 SDK。
3. **扩展加载**：`ExtensionRunner` 用 `jiti` 免编译加载 TypeScript 扩展，扩展注册工具、命令、provider 或 lifecycle hook，并进入 agent runtime。

#### 状态模型

- **持久状态**：jsonl session tree、compaction/branch summary、workspace package lock、coding-agent shrinkwrap、release artifacts。
- **运行时状态**：agent queues、transport、tool execution state、extension instances、provider clients、TUI render state。
- **外部状态**：LLM provider、OAuth/model catalog、宿主文件系统、shell 进程、外部 sandbox/container。jsonl session 是会话事实源，但权限事实源仍是宿主 OS 或外置 sandbox。

#### 契约边界

- **内部契约**：`pi-ai` provider API、`pi-agent-core` Agent interface、SessionManager entry model、Extension lifecycle、TypeBox schemas。
- **外部契约**：`@earendil-works/pi-coding-agent` CLI/package、workspace package APIs、npm shrinkwrap/release smoke。
- **Agent-facing 契约**：tools、session resume/fork/compaction、extension hooks 和 TUI interaction。

#### 失败与降级模型

- README 明确无内建 filesystem/process/network/credential permission system，安全边界必须靠容器、沙箱或宿主权限。
- Node `>=22.19.0` 提高运行环境门槛。
- jsonl session 简单、可审查，但长会话、branch、resume 叠加后会逼近 file-based persistence 上限。
- `jiti` 降低扩展启动成本，同时把部分错误推到运行时。
- exact dependency/shrinkwrap/audit signature 降低供应链风险，也提高升级维护成本。

#### 可复刻设计不变量

1. CLI 产品壳不能吞掉底层 runtime 和 provider substrate。
2. Provider core 要 side-effect-free，历史兼容放在 shim。
3. Session compaction 必须保留 coding 语义，例如读过和改过的文件。
4. Extension lifecycle 要能注册工具、命令、provider 和事件 hook。
5. 无内建权限时必须把安全边界写进产品边界。
6. release engineering 是 agent platform 架构的一部分。
7. 实验 orchestration 应单独包化，避免污染稳定 harness。
8. 文件型持久化要有 locking、versioning 和明确演进路径。
