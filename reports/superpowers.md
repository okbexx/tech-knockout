...
| Star | 248,817（截至 2026-07-08） |
| Fork | 22,096 |
| 许可证 | MIT |
| 语言 | Shell（GitHub 语言主标签）；核心资产仍是 Markdown skills + hooks/scripts |
| 默认分支 | `main` |
| 首次提交 | 2025-10-09 `dd013f6 Initial commit: Superpowers plugin v1.0.0` |
| 最近提交 | 2026-07-02 `d884ae0 Release v6.1.1: fix Codex SessionStart hook re-registration, add Codex portal packaging` |
| 最新 Release | v6.1.1（2026-07-02） |
| Open Issues / PRs | 155 issues / 187 PRs（分别查询 Issues 与 Pulls，不混用 GitHub `open_issues_count`） |
| 已关闭 Issues / 已合并 PRs | 617 issues / 130 PRs |
| 本地规模 | 147 个 tracked files；70 个 Markdown 文件约 17,084 行；31 个 shell 脚本约 5,120 行；6 个 JS/CJS 文件约 1,570 行 |
| 核心贡献 | `git shortlog -sne HEAD` 显示 Jesse Vincent 为绝对主导，Drew Ritter 次之；其余贡献者分散 |
| 分析日期 | 2026-07-08 |
...
- **能做什么：**
  ...
  - **当前 harness 覆盖更广了**：README 最新 quickstart 已覆盖 Claude Code、Antigravity、Codex App、Codex CLI、Cursor、Factory Droid、GitHub Copilot CLI、Kimi Code、OpenCode、Pi。
  - 通过 `.claude-plugin/`、`.codex-plugin/`、`.cursor-plugin/`、`.opencode/`、`.pi/`、`gemini-extension.json`、`hooks/session-start` 等适配不同宿主。
  - 为 OpenCode 做 plugin JS 适配：注册 skills 目录、注入 bootstrap、缓存 bootstrap 内容、映射 Claude 风格工具名到 OpenCode 工具体系。
  - 为 Codex marketplace / portal 维护 plugin manifests 与打包脚本；v6.1.1 明确新增 Codex portal packaging。
  - 为 brainstorming 提供零依赖本地 visual companion server：用 Node 内置 `http/fs/crypto` 实现静态页面、WebSocket、文件监听、生命周期退出。
  - 为复杂技能提供行为测试/集成测试脚本：skill triggering、explicit skill requests、Claude Code skill tests、OpenCode plugin tests、Codex packaging/sync tests、brainstorm server tests。
...
|------------|------|----------|----------------|----------|--------------|---------|
| **无第三方 runtime 依赖** | packaging choice | 整个插件/技能分发层 | 避免把“工作流纪律层”变成安装脆弱、供应链暴露大的 Node 应用 | `package.json` 只有 `name/version/main/pi` 元数据，没有 `dependencies`/`devDependencies` | **极高**：这是它最值得学的取舍之一 | 代价是很多能力必须自己写，不能指望现成生态库 |
| Node built-ins（`http` / `fs` / `crypto` / `child_process` 等） | stdlib runtime | brainstorming visual companion server、hook helpers | 用零依赖方式提供本地可视化与生命周期控制 | `skills/brainstorming/scripts/server.cjs` 与相关脚本 | **高**：适合对可移植性和安装简洁度敏感的插件层项目 | 功能边界会刻意保守，复杂 UI/网络能力不如框架生态 |
| Shell hooks | host integration surface | session-start bootstrap、跨 harness 注入 | 不自造 runtime，只利用宿主已有 hook/plugin/extension 入口 | `hooks/session-start`、各 harness manifests | **高**：对“借宿主、改行为”的项目尤其适用 | 强依赖宿主协议稳定性，平台兼容修复会很多 |
| Host plugin manifests（Claude/Codex/Cursor/OpenCode/Pi 等） | distribution / adapter metadata | 多平台安装、marketplace、bootstrap registration | 把同一 skill corpus 分发到不同宿主而不改核心技能正文 | `.claude-plugin/`、`.codex-plugin/`、`.cursor-plugin/`、`.opencode/`、`.pi/` | **高**：这是 superpowers 变成“跨宿主协议层”的关键 | manifest 漂移、商店规则变化、平台语义差异都会持续带来维护负担 |
...
| Bus factor | 🟡 中 | Jesse Vincent 主导仍非常明显；虽然外部贡献活跃，但方法论口吻与方向集中度高 |
| 供应商锁定 | 🟢 低 | 技能本身是 Markdown，适配多个宿主；可 fork 自建 |
| 运行时攻击面 | 🟡 中低 | 核心无常驻服务；但 hooks、brainstorm server、sync/package 脚本和宿主工具调用仍有副作用边界 |
| Backlog 压力 | 🟡 中高 | 155 open issues + 187 open PRs；相对规模不算爆炸，但对“技能/插件层”来说维护压力并不小 |
| 体验稳定性 | 🟡 中 | v6.1.1 之后 open items 仍集中在 plan 过长、SDD scratch workspace 冲突、token 成本、oh-my-pi 支持等真实使用摩擦 |
| 技能强制力 | 🟡 中 | 依赖 agent 遵守 skill 调用；不是 runtime-enforced policy |
| 贡献治理 | 🟢 强 / 🔴 严 | `CLAUDE.md` 明写 94% PR rejection rate，强力防 slop；质量好，但外部贡献摩擦很高 |
...
- 不建议把它理解为 agent runtime；真正需要自定义 provider、会话持久化、工具权限、HTTP API 的团队，应看 OpenCode / jcode / Pi（原 pi-mono）这类底层 runtime。
...
- **工程化细节强。** 最近一个月新增 / 修补的点集中在 Codex SessionStart hook re-registration、Codex portal packaging、Pi 扩展、Kimi / OpenCode 文档、SDD plan-scoped workspace、visual companion hardening——这说明它处理的是实际 harness 摩擦，而不是写概念文档。
- **零依赖设计自洽。** `package.json` 几乎只保留元数据与入口，brainstorm server 改写为 Node built-ins，是“纪律层尽量不长成 runtime”的自觉取舍。

### 测试

本轮按用户要求只做静态源码分析，未运行项目/测试/构建；以下为源码结构与脚本证据。

- **测试目录规模：** `tests/` 下约 52 个文件。
- **主要测试面：**
  ...
  - `tests/opencode/`：plugin bootstrap caching。
  - `tests/brainstorm-server/`：server、WebSocket protocol、Windows lifecycle。
  - `tests/codex/` + `tests/codex-plugin-sync/`：Codex packaging / marketplace sync。
  - `tests/pi/`、`tests/kimi/`、`tests/antigravity/`：新 harness 适配回归。
- **测试哲学：** 不是覆盖率优先，而是行为证据优先。`CLAUDE.md` 和 `writing-skills` 都把 skill 改动视作行为代码，要求 clean-session transcript、对抗测试或真实 harness 证据。

### CI/CD

- **GitHub Actions：** 当前仓库 `.github/` 下没有 workflow 文件，只有 issue template / funding；质量 gate 主要靠本地脚本、真实 harness transcript、外部 eval harness 和极严的 PR 过滤。
- **版本管理：** `package.json` 版本已到 `6.1.1`；release 通过 `RELEASE-NOTES.md` 与相关脚本驱动。
- **Codex 分发：** `scripts/sync-to-codex-plugin.sh` 与 `scripts/package-codex-plugin.sh` 负责 marketplace / portal 打包与同步。
- **v6.1.1 关键信号：** 项目已经从“多宿主适配”继续演进到“商店/portal 打包 + Pi 扩展 + Kimi 支持 + SDD 产物隔离”这一层。

### 文档质量

- **README：** 安装路径继续扩展，已经覆盖 Antigravity、Codex App、Kimi、Pi 等新宿主。
- **Release Notes：** 信息密度高，能看出每个修复背后的真实平台问题。
- **docs/testing.md：** 明确如何用 Claude Code headless session 和 transcript 验证复杂技能。
- **`CLAUDE.md` + PR template：** 极强约束，要求说明 model/harness/plugins/human partner；新 harness 必须提供 clean-session transcript，且 “Let's make a react todo list” 必须自动触发 brainstorming。

### Issue / PR 健康度

- **热度极高。** 2026-07-08 快照为 248,817 stars / 22,096 forks；fork 比例说明很多人不是只收藏，而是在复制/适配。
- **Backlog 可观。** open issues 155 / open PRs 187；对一个方法论插件来说 PR 压力偏高。
- **治理强硬。** `CLAUDE.md` 明写最近 100 个 closed PR 有 94% rejection，主要针对 AI-generated slop、未读模板、重复 PR、没有 human review、没有真实问题描述等。
- **生态外溢明显。** GitHub 搜索可见 `superpowers-zh`、`superpowers-skills`、`superpowers-marketplace` 等衍生项目。
...
| 社区活跃度 | 5 | 248k+ stars、22k+ forks，生态外溢明显 |
...
- 用 `CLAUDE.md` / PR template 解决“agent 参与开源贡献时如何防 slop”。
