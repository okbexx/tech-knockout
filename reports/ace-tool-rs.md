...
这次继续往下读源码后，可以把它的 backend contract 说得更硬一些：

1. **`search_context` 不是本地语义搜索，而是远端检索代理。**
   - `src/index/manager.rs::search_context()` 会先本地 `index_project()`，拿到 blob hash 列表。
   - 然后直接 POST 到：`{base_url}/agents/codebase-retrieval`
   - 请求体核心字段是：
     - `information_request`
     - `blobs.added_blobs`
     - `disable_codebase_retrieval`
     - `enable_commit_retrieval`
   - 响应只读取 `formatted_retrieval`。
   - 这说明它本地并不做完整 retrieval ranking，只负责把“代码切块 + blob 集合”喂给远端约定接口。

2. **prompt enhancement 明显带有 Augment 兼容层痕迹。**
   - `src/service/augment.rs` 里有两条原生路线：
     - `POST {base_url}/prompt-enhancer`
     - `POST {base_url}/chat-stream`
   - `src/enhancer/prompt_enhancer.rs` 文件头直接写着 `Based on Augment VSCode plugin implementation`。
   - `src/index/manager.rs` 的 blob 大小、batch 大小注释也写着 `aligned with official augment.mjs`。
   - `src/service/common.rs` 还会把返回文本里的 `codebase-retrieval` 重写成 `search_context`，并识别 `<augment-enhanced-prompt>...</augment-enhanced-prompt>` 标签。
   - 这几处合起来，已经足以判断：**它不是抽象地“支持任意后端”，而是先围绕 Augment 风格协议建模，再外扩到第三方 API。**

3. **第三方 API 支持，本质上是“增强器替换”，不是“检索后端替换”。**
   - `claude` → `POST /v1/messages`
   - `openai` → `POST /v1/chat/completions`
   - `codex` → `POST /v1/responses`
   - `gemini` 也是对应 Google 路线
   - 但这些只覆盖 `enhance_prompt` 这条链。
   - 真正的 `search_context` 仍然要求 `base_url + token + /agents/codebase-retrieval` 这套专有契约存在。

4. **README 对“后端来源”仍然没有完全说穿。**
   - 文档告诉你要填 `--base-url` / `--token`，也告诉你第三方增强端点怎么切。
   - 但没有真正把“如果没有 Augment/兼容 retrieval 服务，你将失去哪部分核心能力”讲透。
   - 这就是为什么 issue 里会出现“Augment 官方 url 到底填什么”和“能不能不用公益服/augment key、自建平替后端”这类问题。

**归纳一句：**
ace-tool-rs 的真实产品边界，不是“完整代码智能引擎”，而是 **“Augment 风格代码检索协议 + 多家 LLM prompt enhancement API 的本地 MCP 适配器”**。
