...
> 一句话定位：**Compound Engineering 是 EveryInc 把“复利工程”编码成 root-native 多宿主 AI coding workflow 插件：以顶层 `skills/` 和多份 native plugin manifest 为事实源，向 Claude、Cursor、Codex、Kimi 与 adapter targets 分发，并用 brainstorm → plan → work → simplify → review → compound 的工程闭环把团队 AI coding 方法论产品化。**
...
| Star | 22,816（2026-07-08 GitHub API 快照） |
| Fork | 1,703（2026-07-08 GitHub API 快照） |
| Watchers | 116（2026-07-08 GitHub API 快照） |
| 许可证 | MIT |
| 语言 / 运行时 | TypeScript + Markdown；Bun / Node.js 22；root-native plugin manifests + target adapters |
| 默认分支 | `main` |
| GitHub created_at | 2025-10-09 |
| 首次提交 | 2025-10-09 `77de729 Kieran Klaassen Initial commit: Every Marketplace for Claude Code` |
| 最近提交 | 2026-07-08 `1756c0b9f3cf94493f287ea29ae766ad668fb7cf chore: release main (#1075)` |
| 最新 GitHub Release | `compound-engineering-v3.19.0`（2026-07-08） |
| Repo package / npm | `origin/main:package.json` 已是私有 `compound-engineering@3.19.0`；公开 npm `@every-env/compound-plugin` latest 仍停在 `3.8.3`，registry `modified=2026-05-18T16:35:38.583Z` |
| Open Issues / PRs | 63 issues / 31 PRs（GitHub Search API；仓库 API `open_issues_count=94` 含 PR） |
| 贡献者 | GitHub contributors API 估算 72；top 5 为 `tmchow` 467、`kieranklaassen` 247、`github-actions[bot]` 80、`semantic-release-bot` 25、`mvanhorn` 23 |
| 远端规模 | `origin/main` 627 tracked files；117 `.ts`、425 `.md`、66 个 `tests/**/*.test.ts`、29 个 `skills/*/SKILL.md`、0 个 standalone agents、3 个 workflows |
| 本地 source cache 状态 | 工作树 `HEAD` 仍停在 2026-06-15 `2648200e`；`git fetch --tags` 后相对 `origin/main` 为 `0/90`，旧本地快照已明显落后 |
| 分析日期 | 2026-07-08 |
...
- 提供一套完整 CE workflow：`ce-strategy`、`ce-ideate`、`ce-brainstorm`、`ce-plan`、`ce-work`、`ce-simplify-code`、`ce-code-review`、`ce-doc-review`、`ce-debug`、`ce-compound`、`ce-product-pulse`、`lfg` 等，当前 README 口径为 **29 skills**。
- 把顶层 `skills/`、`.claude-plugin` / `.codex-plugin` / `.cursor-plugin` / `.kimi-plugin` / `plugin.json` 作为 root-native 资产层，同时保留 `src/parsers` + `src/converters` + `src/targets` 作为 adapter/runtime compatibility layer。
- 对 Claude、Cursor、Codex、Kimi 提供 native plugin manifest；对 OpenCode、Codex、Pi、Antigravity 保留 converter + writer 路径，兼容非统一宿主能力面。
- 用 managed manifest、backup、safe path filtering、hook merge index、bounded config merge 控制安装/重装/清理副作用，而不是盲目覆盖宿主目录。
- 用真实插件转换 drift tests 验证 shipping plugin 到每个已实现 target 的结构漂移，而不是只测 toy fixture。
- 将 review、research、计划和复盘规则收进各自 skill 目录里的本地 prompt / reference 资产；当前 README 明写 **0 个 standalone agents**，专项行为更多以内嵌 skill-local assets 形态存在。
...
- 不是轻量“装上就无感”的个人插件：尽管 standalone agents 已消失，但 29 skills、artifact-heavy loop 和 review/compound 产物仍有明显心智与上下文成本。
- 对平台能力依赖依旧很强：native plugin 面扩大了，但不同宿主的 capability surface 仍不统一；非原生 target 继续依赖 converter / writer / managed config merge。
- 发布与分发口径仍需单独核验：repo manifests / release / root package 已到 `3.19.0`，公开 npm `@every-env/compound-plugin` 却仍停在 `3.8.3`，说明分发层存在长期漂移风险。
...
- **依赖链低。** runtime dependencies 仍只有 `citty` 和 `js-yaml`；开发/测试/发布主要依赖 Bun 与 semantic-release。
- **安装路径是 hybrid，不再是单一路径。** Claude、Cursor、Codex、Kimi 已有 native plugin manifest；OpenCode / Pi / Antigravity 仍依赖 converter + writer；repo 同时承担“first-class plugin”与“compatibility layer”双角色。
- **真实副作用仍存在。** 非原生 target writer 依旧会写宿主 home/config（如 `.codex/config.toml`、hooks、skills、prompts、plugin-owned files），managed manifest / backup / bounded merge 只是降低风险，不是消灭风险。
- **学习曲线中高。** 用户要理解 CE loop、技能边界、产物目录、review/simplify/compound 的职责划分；不是只多一个 slash command。
- **从零到 demo 更快了，但团队采用仍不轻。** Claude/Cursor/Codex/Kimi 的 native install 降低了首装摩擦；真正进入团队流程时，仍要约定哪些 skill 必跑、哪些产物必须留档、review finding 如何进入 PR/issue/solution。
...
|------------|------|----------|----------------|----------|--------------|---------|
| `citty` | runtime package | CLI command surface (`install` / `convert` / `list` / `cleanup`) | 用极薄命令层把解析、参数校验和 composition root 固定下来 | `origin/main:package.json` `dependencies.citty`；`src/index.ts` | 适合 agent tooling / installer 类 TypeScript CLI | 只解决命令面，不替代真正的 installer policy |
| `js-yaml` | runtime package | frontmatter / YAML 解析 | 让 `SKILL.md` 和 manifest 周边元数据可结构化读写 | `origin/main:package.json` `dependencies["js-yaml"]` | 适合 Markdown-first / skill-first 系统 | YAML 契约漂移仍需自家校验与测试兜底 |
| Bun | toolchain | `bun run` / `bun test` / release scripts | 统一 TS 执行、测试与脚本运行，降低脚手架体积 | `origin/main:package.json` scripts 全面使用 `bun` | 适合薄 runtime + 大量 Markdown 资产仓库 | 团队环境需统一 Bun 版本，Windows / CI 细节要额外注意 |
| `semantic-release` + release validation | release stack | 多 manifest / 多宿主插件版本推进 | 降低 root package、plugin manifests 与 release PR 的人工同步成本 | `origin/main:package.json` devDependencies；`src/release/`；Actions `Release PR` | 适合多分发表面的插件仓库 | 不能自动解决 npm channel 停滞；外部分发仍可能漂移 |
...
| Bus factor | 中 | contributors API 估算 72，但提交仍高度集中在 `tmchow` 与 `kieranklaassen`；方向受 Every 团队主导。 |
| 供应商锁定 | 中 | 文本资产可迁移，但 first-class 格式是 Claude Code plugin；其他平台需要转换/适配。 |
| 维护趋势 | ✅ 极活跃 | 2026-07-08 仍在发 `compound-engineering-v3.19.0`；本地旧 clone 相对 `origin/main` 已落后 90 commits，说明版本推进很快。 |
| 发布口径 | 中高 | repo manifests / root package / GitHub release 已到 `3.19.0`，公开 npm `@every-env/compound-plugin` latest 却仍是 `3.8.3`；采用前必须区分“哪条分发面是你真正安装的”。 |
| 运行时副作用 | 中 | native plugin 面扩大降低了部分安装摩擦，但 converter/writer 仍会改宿主配置与 managed artifacts；安全路径过滤较强，仍建议 isolated profile。 |
| 上下文成本 | 中高 | 29 skills + review / simplify / compound pipeline 仍会增加 token 与时延；适合高价值任务，不适合每个微小改动都全流程。 |
...
- CE 的核心价值不在“又一堆 prompts”，而在把 brainstorm、plan、work、simplify、review、compound 做成连续 workflow，并把工程产物写回项目，形成下次任务还能复用的 grounding。
- 它对已经重度使用 AI coding 的团队很有价值：能降低“每次 agent 重新学习一次项目”的浪费，也能用 review / doc-review / compound pipeline 抑制盲目自动改代码。
- 架构上它已经从“Claude-first skill pack + converter”进一步演成 **root-native plugin + compatibility layer**：native plugin 面扩大降低了 Claude/Cursor/Codex/Kimi 的首装摩擦，但也让分发口径治理更复杂。
- 它仍不是最轻方案。个人用户只想让 agent 遵守基本纪律，`superpowers` 更轻；团队想统一多 harness 安装/治理，`ECC` 更系统；想做 recurring maintenance loop，`loop-engineering` 更直指问题。
- 采用方式建议：先在隔离 profile / throwaway repo 试 `/ce-brainstorm → /ce-plan → /ce-work → /ce-simplify-code → /ce-code-review → /ce-compound`，再决定哪些环节进入团队默认流程。若通过 native plugin 安装，要确认 marketplace 已刷新到 `3.19.0` 线；若走 converter / writer，要额外检查 managed files 和 bounded config merge 的落点。
...
│      root-native plugin manifests + skill corpus + adapter runtime    │
└──────────────────────────────────┬──────────────────────────────────┘
                                   │
┌──────────────────────────────────▼──────────────────────────────────┐
│                    Root-native Asset / Manifest Plane                 │
│ .claude-plugin / .codex-plugin / .cursor-plugin / .kimi-plugin       │
│ plugin.json (Antigravity) / top-level skills/*/SKILL.md              │
│ docs/skills/* / skill-local prompt & reference assets                │
...
│ manifest + commands + skills + hooks + mcpServers (+ generic agents) │
└──────────────────────────────────┬──────────────────────────────────┘
                                   │ routed by
┌──────────────────────────────────▼──────────────────────────────────┐
│                         Target Registry                              │
│ src/targets/index.ts                                                 │
│ opencode / codex / pi / antigravity                                  │
...
│ - native-plugin / prompt / skill dir / optional agent sidecar map    │
└──────────────────────────────────┬──────────────────────────────────┘
                                   │ writes with
┌──────────────────────────────────▼──────────────────────────────────┐
...
┌──────────────────────────────────▼──────────────────────────────────┐
│                         Host Harnesses                               │
│ Claude Code / Cursor / Codex / Kimi (native)                         │
│ OpenCode / Pi / Antigravity (adapter-written bundles)                │
...
> **Agent-facing Workflow Corpus + Root-native Plugin Manifests + Skill-local Specialist Prompt Assets + Target Adapter Registry + Managed Artifact State + Platform-specific Content Transformer + Review/Compound Feedback Contract**

这几个抽象缺一不可：

1. **Workflow Corpus** 定义 agent 行为：什么时候问、什么时候计划、什么时候简化、什么时候审查、什么时候沉淀。
2. **Skill-local Specialist Prompt Assets** 把 reviewer / research / doc-review 等专项行为嵌在 owning skill 内，而不是依赖庞大的独立 agent 目录。
3. **Source Plugin Model** 给跨平台分发一个事实源：root-native plugin manifests + `skills/` 是当前主资产层，避免每个平台手写一套资产。
...
| 抽象 | 文件 / 目录 | 职责 | 为什么重要 |
|------|-------------|------|------------|
| ClaudePlugin | `src/types/claude.ts`、`src/parsers/claude.ts` | 统一承载 manifest、commands、skills、hooks、mcpServers（模型层仍兼容 generic agents） | 所有 target converter 的共同输入模型，决定系统能否跨平台扩展 |
| Skill | `skills/*/SKILL.md` | agent-facing workflow contract：触发、阶段、问题、文件产物、验证、边界 | CE 真正的 runtime interface，不是辅助文档 |
| Skill-local Specialist Prompt Asset | `docs/skills/*` 与 skill 邻近 assets | 把 reviewer / research / explain 等专项行为绑在 owning skill 上 | 说明当前 CE 的质量来自 skill contract + local assets，而不是 standalone agent catalog |
...
- `.claude-plugin/plugin.json`、`.codex-plugin/plugin.json`、`.cursor-plugin/plugin.json`、`.kimi-plugin/plugin.json`、`plugin.json`：声明 root-native plugin 元数据、skills 目录和平台 surface。
...
- `src/converters/*` 转换 skill/command 内容与引用名，并保留 generic agent conversion capability。
- `src/targets/*` 写入目标宿主目录，复制 sidecar、生成 TOML/JSON/Markdown、合并 config/hooks。
- `skills/*/references/` 与 scripts 是 agent 执行时读取的知识/脚本数据。
...
- **Developer CLI 契约：**
  ...
  - repo-local `bun run src/index.ts install <plugin> --to <target> [--branch] [--codex-home] [--include-skills]`
  - `--to all` 只安装检测到且 implemented 的 adapter target；对 native plugin 宿主，终端安装路径更多由 manifest/marketplace 承载。
- **TargetHandler 契约：** 每个 target 必须提供 `convert` 与 `write`；scope 支持必须声明，否则 `--scope` 被拒绝。
- **Plugin manifest 契约：** `.claude-plugin` 是 source-compatible manifest；`.codex-plugin`、`.cursor-plugin`、`.kimi-plugin`、`plugin.json` 分别暴露各宿主 native/plugin surface。
- **Skill 契约：** `skills/<name>/SKILL.md` frontmatter 的 `description` 是触发面，正文是执行协议；`ce_platforms` 限定平台可见性。
- **Specialist prompt asset 契约：** 当前 CE 的 review/research 专项行为更多以内嵌 skill-local prompt assets 与 `docs/skills` 文档显式定义，而不是独立 `agents/*.md` 目录。
...
5. **多 specialist review 要有角色、路由和合成规则。** 并发本身不创造质量；reviewer taxonomy、severity、confidence、autofix_class、owner 才是质量控制点。
...
9. **native 能力优先，converter 补缺口。** Native plugin 能承载的能力不要重复安装；converter 负责 compatibility layer、fallback 和 non-native target 写入。
...
├── package.json                      # root package（当前 version=3.19.0，private）
├── .claude-plugin/                   # Claude-compatible source/native manifest
├── .codex-plugin/                    # Codex native plugin manifest
├── .cursor-plugin/                   # Cursor native plugin manifest
├── .kimi-plugin/                     # Kimi native plugin manifest
├── plugin.json                       # Antigravity plugin manifest
├── skills/                           # 29 个 SKILL.md workflow + references/scripts
├── docs/skills/                      # 面向用户的技能说明目录
├── src/
│   ├── index.ts                      # citty CLI 入口
│   ├── commands/                     # install / convert / list / cleanup
│   ├── parsers/                      # Claude-compatible plugin parser
│   ├── converters/                   # Claude → OpenCode / Codex / Pi / Antigravity
...
├── tests/                            # 66 个 bun:test 文件
├── scripts/release/                  # release sync / preview / validate
├── .github/workflows/                # 3 条 workflows（CI / release / pages）
...
- **CLI 框架：** `citty`。
- **配置/文档格式：** Markdown + YAML frontmatter；JSON manifests（Claude/Codex/Cursor/Kimi/Antigravity）；TOML for Codex agent/config 输出。
- **测试：** Bun 内置 test runner，`tests/**/*.test.ts` 66 个。
- **CI：** GitHub Actions 主要是 CI / Release PR / Pages 三条 workflow；最近 12 条 runs 以 success 为主，但 release-please 分支仍可见 `action_required` 信号。
...
2. **新增 skill：** 在 `skills/<name>/SKILL.md` 写 frontmatter + body；可用 `ce_platforms` 限定平台。
3. **新增 specialist behavior：** 当前更推荐把 reviewer/research 等行为内嵌到 owning skill 的本地 prompt/reference assets；generic agent model 仍存在，但不是 CE 当前主资产形态。
4. **新增 hook/MCP：** 通过 root-native plugin manifests 或 default `hooks/hooks.json`、`.mcp.json` 被 parser 读取，并由 target writer merge。
...
- **Codex 兼容细节扎实。** hybrid native/converter 路径、externallyManagedSkillNames、legacy symlink ownership、agent filename collision 检查，都是从真实安装痛点演进出的设计。
...
- **asset 面仍然很大。** 即使 README 口径已收敛到 29 skills、0 standalone agents，skill-local prompt/reference/doc 资产依然庞大；修改一条 workflow 规范仍可能同步影响多处说明与 target variants。
...
- **测试文件规模：** `tests/**/*.test.ts` 66 个。
- **主要测试面：**
  - parser/frontmatter/manifest path safety。
  - converter 输出：OpenCode、Codex、Pi、Antigravity。
  - target writer：config merge、hooks merge、managed artifacts、legacy cleanup、path safety。
  - release config validation。
  - real plugin conversion drift：真实 repo-root plugin assets 转所有 implemented target，并按 source inventory 校验 commands/skills/optional agents 数量和结构。
...
- **测试哲学：** 对 converter/writer 这类可确定代码做结构性测试；对 skills/prompt assets 这种 agent-facing 文本资产，用 real plugin drift test 防格式/数量漂移。
...
- **多组件版本：** root package、`.claude-plugin`、`.codex-plugin`、`.cursor-plugin`、`.kimi-plugin`、`plugin.json` 都要一起对齐；当前远端 manifest 线已到 `3.19.0`。
- **stale release-as 防线：** `src/release/config.ts` 会检测 `release-as` 是否不高于 base manifest released version，防止一次性 pin 遗留后冻结后续 release。
- **当前发布风险：** GitHub latest release 已到 `compound-engineering-v3.19.0`，但 npm registry latest 仍是 `3.8.3`；另外最近 12 条 Actions runs 虽以 success 为主，但 `release-please--branches--main` 仍出现 `action_required`。

### 文档质量

- **README 使用路径完整。** Claude Code、Cursor、Codex App/CLI、Kimi 等 native 路径写得清楚，也明确提示 existing installs 需要 refresh marketplace。
- **docs/skills 目录成体系。** `docs/skills/README.md` 把 core loop / around-the-loop / on-demand / autonomous pipeline 分层写清楚，并声明 `skills/<skill>/SKILL.md` 才是 authoritative runtime spec。
...
- **活跃度高。** 2026-07-08 查询：open issues 63、open PRs 31；当天仍在发 `v3.19.0`，CI 与 Release PR 持续滚动。
...
- **维护响应快。** 2026-07-08 `origin/main` 最新提交就是 `chore: release main (#1075)`，多条 PR run 在同日上午完成。
- **Backlog 不小但可解释。** 项目处在 native plugin 扩面 + compatibility layer 继续维护的阶段，open issues 中大量是具体 workflow/compat 问题，不是无人维护迹象。
...
- **技术要点：**
  ...
  - 当前 implemented targets：OpenCode、Codex、Pi、Antigravity。
  - Claude/Cursor/Codex/Kimi 同时存在 native plugin surface；`src/targets` 更多体现 compatibility / fallback / power-user writer 路径。
- **学习价值：** 小接口控制平台差异，是 CE 分发架构能扩展的核心。

### 3. `src/converters/claude-to-codex.ts` — Codex hybrid conversion policy

- **职责：** 把 ClaudePlugin 转为 Codex bundle。
- **关键设计：** generic converter 默认 agents-only；`--include-skills` 才进入 full mode，但当前 CE repo 的终端用户路径更偏 native plugin install。
- **技术要点：**
...
### 6. `skills/ce-code-review/SKILL.md` — 多 specialist review workflow
...
- **技术要点：**
  ...
  - 合成阶段强调去重、保守路由和验证责任。
- **学习价值：** 多 specialist review 不是“多叫几个 agent”，而是 reviewer taxonomy + structured finding contract + synthesis policy。

### 7. `skills/ce-plan/SKILL.md` — 计划 contract
...
- **技术要点：**
  - source inventory 独立从 repo root 的 manifests + `skills/` 读取，而不是复用 converter 输出。
...
| 测试质量 | 4 | 66 个 Bun tests，真实插件转换 drift test 很好；本轮未实跑，workflow 文本行为仍难靠测试完全覆盖。 |
| 文档质量 | 5 | README、`docs/skills`、`SKILL.md` 都很强；缺点是快速演进带来局部文档漂移风险。 |
...
**Compound Engineering 是目前最值得拆解的团队型 AI coding workflow 插件之一：它的价值不只是 29 个 skills，而是把“需求 → 计划 → 执行 → 简化 → 审查 → 复利沉淀”做成 agent 可执行、可分发、可升级的系统。**
...
- 想把内部 agent workflow 做成可分发插件 / skills / compatibility layer 的平台工程师。
...
4. **确认安装源。** 2026-07-08 repo manifests / release 已到 `3.19.0`，公开 npm channel 仍停在 `3.8.3`；采用前先分清你实际装的是 native marketplace、GitHub source 还是历史 npm 包。
