...
| Star | 18,443（观测：2026-07-08） |
| Fork | 1,253（观测：2026-07-08） |
| Watchers | 51（观测：2026-07-08） |
| 许可证 | AGPL-3.0-or-later |
| 主要语言 | TypeScript（Tauri/React 前端 + Rust 后端 + Node MCP server） |
| 首次提交 | 2026-02-14 |
| 最近提交 | 2026-07-07 `20ead5ad` |
| 最新 stable release | `v2026-07-01`（2026-07-01） |
| 最新 alpha tag | `alpha-v2026.7.7-alpha.0003` |
| 贡献者数 | 24（GitHub contributors API `anon=1` 分页口径） |
| 当前健康信号 | open pure issues 32 / open PRs 42；最近 PR `CI` 多为 `action_required`，但 `Release (Alpha)` 与 `Auto-update PR branches` 在 main push 上 success |
| 分析日期 | 2026-07-08 |
...
- **本次验证边界：** 这轮按 TK 静态源码口径只做 GitHub API、远端 fetch、README/关键源码/CI 新鲜度复核；未在本地跑 `pnpm` / `cargo` / Playwright。
...
| Bus factor | ⚠️ 中 | GitHub contributors API 口径 24 人，但提交高度集中在 Luca / Refactoring 主线；好处是产品意志强，坏处是关键方向依赖单一核心。 |
...
| 维护趋势 | ✅ 极活跃 | 旧 clone 相对远端已落后 239 commits；stable 已推到 `v2026-07-01`，alpha tag 持续滚到 `alpha-v2026.7.7-alpha.0003`。 |
| 安全历史 | ⚠️ 中 | 有 `SECURITY.md`、CSP、路径 boundary、Sentry/PostHog scrub；但最近 PR `CI` 多个结论为 `action_required`，公开自动安全门仍不算完整。 |
...
- 最近 12 条 Actions runs 里，`Release (Alpha)` 与 `Auto-update PR branches` 在 main push 上 success；大量 PR `CI` 结论为 `action_required`，说明仓库活跃但 review/approval/runner gating 也带来明显流程摩擦。
...
- open pure issues：32；open PRs：42（GitHub search API，2026-07-08）。
- 本地旧 clone `HEAD` 停在 2026-06-13 的 `b92d0c7d`，远端 `origin/main` 已推进到 2026-07-07 的 `20ead5ad`，ahead/behind 为 `0/239`。
- stable release 已到 `v2026-07-01`，alpha tag 继续滚到 `alpha-v2026.7.7-alpha.0003`；这说明项目仍处在极高频产品化阶段。
- 最近 PR `CI` 大量为 `action_required`，说明外部贡献和分支活动活跃，但 review/runner/approval 吞吐也在承压。
- Issue 类型仍然很真实：Windows path/MCP、Linux media/PDF、IME、中国语言、Git identity、wikilink 等，符合跨平台桌面 + Git + AI 工具链产品的真实故障面。
...
- 创建不到 5 个月已到 18k+ stars，说明传播能力仍很强；`docs/VISION.md` 和 Refactoring newsletter 仍是重要分发渠道。
- stable/alpha 双轨 release 继续高频滚动，alpha tag 总量已破 1000，说明项目已进入真实密集发布状态，而不是偶发演示版节奏。
- Issue 大多是实际使用问题，不是空泛 feature wish；这说明项目已经进入真实用户试用阶段。
- 维护者响应快、commit/release 频繁，但 PR backlog 和 `action_required` CI 也说明社区治理/审批流仍在磨合。
